...
"""UniGS pipeline on PixArt-α (context-token concat, epsilon diffusion)."""
...
def _import_pixart():
    ...
    return PixArtTransformer2DModel, DPMSolverMultistepScheduler


class UniGSPixArtPipeline(DiffusionPipeline):
    """UniGS on [`PixArtTransformer2DModel`] (PixArt-α).

    Image and colormap latents are denoised jointly with context-token concat
    after patch embed. Learned-sigma halves are dropped before the scheduler.
    """
    ...
    def __init__(
        ...
        self.register_to_config(requires_safety_checker=requires_safety_checker, unigs_dit_family="pixart")
        self.vae_scale_factor = 2 ** (len(self.vae.config.block_out_channels) - 1) if getattr(self, "vae", None) else 8
        self.latent_channels = int(getattr(self.vae.config, "latent_channels", None) or 4)
    ...
    def encode_prompt(
        ...
        max_sequence_length = max_sequence_length or default_dit_max_sequence_length("pixart")
    ...
    def _predict(self, image, colormap, control, mask, timestep, encoded, guidance_scale: float, height: int, width: int):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if guidance_scale is None:
            guidance_scale = default_dit_guidance("pixart")
        if prompt is None:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                image_in = self.scheduler.scale_model_input(image_latents, t)
                colormap_in = self.scheduler.scale_model_input(colormap_latents, t)
                current = t
